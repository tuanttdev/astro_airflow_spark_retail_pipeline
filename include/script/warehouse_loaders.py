...
## ---- POSTGRES SOURCE AND STAGING DATABASE ----
PG_URL  = f"jdbc:postgresql://{os.getenv('PG_HOST','localhost')}:5432/{os.getenv('PG_DB','retaildb')}"
...
def ensure_wh_schema():
    ...
    ch_exec("""
        CREATE TABLE IF NOT EXISTS retail.dim_channel(
            ...
            monthly_fixed_cost Decimal(18,2),
...
def load_dim_channel_spark(run_date: str):
    ...
    new_dim = new_dim.fillna(value=UNKNOWN_STRING_VALUE)
    new_dim = new_dim.fillna(value=UNKNOWN_NUMBER_VALUE)
...
def load_dim_product_spark(run_date: str):
    ...
    df = (_read_pg_query(s, f"""
        ...
        "supplier",
        "product_status",
        F.col("stock").cast(T.IntegerType()),
        F.col("cost_price").cast(T.DecimalType(18, 2)),
        F.col("unit_price").cast(T.DecimalType(18, 2)).alias("selling_price")
    ...
    dim_product = (_read_ch_query(s, f"""
                        ...
                        from dim_product where to_date is null """))

    #

    join_condition = """dim.product_id = ch.product_id
                            AND dim.product_name = ch.product_name
                            AND dim.category = ch.category
                            AND dim.brand = ch.brand
                            AND dim.supplier = ch.supplier
                            AND dim.cost_price = ch.cost_price
                            AND dim.selling_price = ch.selling_price
                            AND dim.profit_margin = ch.profit_margin
                            AND dim.product_status = ch.product_status
                            """

    joined = df.alias('ch').join(
        dim_product.alias('dim'),
        on=F.expr(join_condition),
        how='inner').select(F.col("ch.product_id"),
                            "dim.product_key", 'ch.product_name', 'ch.category', 'ch.brand',
                            'ch.supplier', 'ch.stock', 'ch.cost_price', 'ch.selling_price', 'ch.profit_margin', 'ch.product_status'
                            , "dim.from_date", "dim.to_date")
    #
    ...
    new_records = df.alias('ch').join(
        dim_product.alias('dim'),
        on=F.expr(join_condition),
        how='left').filter("dim.product_key is null ").select(F.col("ch.product_id"),
                                                                "ch.product_key", 'ch.product_name', 'ch.category', 'ch.brand',
                                                                'ch.supplier', 'ch.stock', 'ch.cost_price', 'ch.selling_price', 'ch.profit_margin', 'ch.product_status'
                                                              , "ch.from_date", F.lit(None).alias("to_date"))
    #

    old_records = dim_product.alias('dim').join(
        df.alias('ch'),
        on=F.expr(join_condition),
        how='left').filter("ch.product_key is null ").select(F.col("dim.product_id"),
                                                             "dim.product_key", 'dim.product_name', 'dim.category', 'dim.brand',
                                                                'dim.supplier', 'dim.stock', 'dim.cost_price', 'dim.selling_price', 'dim.profit_margin', 'dim.product_status'
                                                             , "dim.from_date",
                                                             F.date_sub(F.current_date(), 1).alias("to_date"))
    #
